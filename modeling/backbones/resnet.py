...
class DeepthConv(nn.Module):
    ...
    def __init__(self, inplanes, planes, gcb, se, stride=1):
        ...
        if self.with_se:
            self.se_module_1 = SELayer(channel=planes * 4)
            self.se_module_2 = SELayer(channel=planes * 4)
    ...
    def forward(self, x):
        ...
        if self.with_se:
            out_1 = self.se_module_1(out_1)
            out_2 = self.se_module_2(out_2)
