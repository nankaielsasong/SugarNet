...
class DeepthConv_6(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, gcb, stride=1):
        super(DeepthConv_6, self).__init__()
        self.with_gcb = gcb
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.channel_num = planes // 6 + 1

        self.dep_conv1 = nn.Conv2d(self.channel_num, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.dep_conv2 = nn.Conv2d(self.channel_num, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.dep_conv3 = nn.Conv2d(self.channel_num, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.dep_conv4 = nn.Conv2d(self.channel_num, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.dep_conv5 = nn.Conv2d(self.channel_num, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.dep_conv6 = nn.Conv2d(planes - 5 * self.channel_num, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        ...
        self.bn2_5 = nn.BatchNorm2d(planes)
        self.bn2_6 = nn.BatchNorm2d(planes)
        ...
        self.conv3_5 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        self.conv3_6 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        
        self.bn3_1 = nn.BatchNorm2d(planes * 4)
        self.bn3_2 = nn.BatchNorm2d(planes * 4)
        self.bn3_3 = nn.BatchNorm2d(planes * 4)
        self.bn3_4 = nn.BatchNorm2d(planes * 4)
        self.bn3_5 = nn.BatchNorm2d(planes * 4)
        self.bn3_6 = nn.BatchNorm2d(planes * 4)
        ...
        if self.with_gcb:
            ...
            self.context_block_5 = ContextBlock(inplanes=gcb_inplanes)
            self.context_block_6 = ContextBlock(inplanes=gcb_inplanes)
    ...
    def forward(self, x):
        ...
        out_1 = out[:, :self.channel_num]
        out_2 = out[:, self.channel_num:2*self.channel_num]
        out_3 = out[:, 2*self.channel_num:3*self.channel_num]
        out_4 = out[:, 3*self.channel_num:4*self.channel_num]
        out_5 = out[:, 4*self.channel_num:5*self.channel_num]
        out_6 = out[:, 5*self.channel_num:]
        ...
        out_5 = self.dep_conv5(out_5)
        out_5 = self.bn2_5(out_5)
        out_5 = self.relu(out_5)
        out_5 = self.conv3_5(out_5)
        out_5 = self.bn3_5(out_5)

        out_6 = self.dep_conv6(out_6)
        out_6 = self.bn2_6(out_6)
        out_6 = self.relu(out_6)
        out_6 = self.conv3_6(out_6)
        out_6 = self.bn3_6(out_6)
        ...
        if self.with_gcb:
            ...
            out_5 = self.context_block_5(out_5)
            out_6 = self.context_block_6(out_6)
