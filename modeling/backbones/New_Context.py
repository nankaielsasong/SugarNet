...
class ContextBlock(nn.Module):
    ...
    def spatial_pool(self, x):
        ...
        if self.pooling_type == 'att':
            ...
            # [N, 1, H, W]
            context_mask = self.c_pooling(x)# self.conv_mask(x) + self.c_pooling(x)
            # [N, 1, H * W]
    ...
    def forward(self, x):
        ...
        # context = self.avg_pool(x) + self.max_pool(x)
        out = x
        if self.channel_mul_conv is not None:
            # [N, C, 1, 1]
        ...
        if self.channel_add_conv is not None:
            # [N, C, 1, 1]
            channel_term = self.channel_add_conv(context)
            out = out + channel_term
        return out
