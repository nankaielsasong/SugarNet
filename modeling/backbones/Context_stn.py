...
class ContextBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc_loc = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(self.planes, 3 * 2)
        )
        self.fc_loc.apply(weights_init_classifier)
    ...
    def forward(self, x):
        ...
        if self.channel_add_conv is not None:
            ...
            channel_add_term = channel_add_term.view(channel_add_term.shape[0], -1)
            stn_out = self.stn(out, channel_add_term)
            out = out + stn_out
